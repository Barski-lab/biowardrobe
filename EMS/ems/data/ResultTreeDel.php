...
function del_data($data,$con) {
    ...
    if($data->leaf==true) {
        ...
        $result=execSQL($con,"SELECT tableName FROM result WHERE labdata_id is NULL and ahead_id in (select ahead_id from analysis where rhead_id =?)",array("i",$data->parentId),false);
        if($result != 0 )
