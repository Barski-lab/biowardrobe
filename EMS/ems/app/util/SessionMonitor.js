 ...
 * logged out.
 */
Ext.define('EMS.util.SessionMonitor', {
    singleton: true,

    interval: 1000 * 10,  // run every 10 seconds.
    lastActive: null,
    maxInactive: 1000 * 60 * 30,  // 30 minutes of inactivity allowed; set it to 1 for testing.
    remaining: 0,
    panel: {},
    ui: Ext.getBody(),
        ...
        buttons: [
            {
                ...
                handler: function () {
                    Ext.TaskManager.stop(EMS.util.SessionMonitor.countDownTask);
                    EMS.util.SessionMonitor.window.hide();
                    EMS.util.SessionMonitor.start();
    ...
    monitorUI: function () {
        ...
        var clock = (this.maxInactive-inactive) / (1000 * 60)+1;

        this.panel.setTitle('<div style="float: right; text-align: right;">' + Ext.Date.format(now, 'g:i:s A') + '&nbsp;(' + clock.toFixed() + ')&nbsp;</div>');
