...
$UID = $_REQUEST['UID'];
check_val($UID);
$path=$settings->settings['wardrobe']['value'].$settings->settings['preliminary']['value'].'/'.$UID.'/';
$url=$settings->settings['preliminary']['value'].'/'.$UID.'/';

#logmsg($_REQUEST);
			
// change these parameters to fit your server config
$allowedFormats    = "jpg,jpeg,gif,png";			// Allowed image formats
$maxSize           = "10485760";				// Max image size. 10485760 = 10MB
$imagesPath        = $path; 					// path where the files will be uploaded to the server
$imagesThumbsPath  = $path."thumbs/";				// path where the image thumbs will be uploaded to the server
$imagesTumbsUrl    = $url."thumbs/";
$imagesUrl         = $url; 
