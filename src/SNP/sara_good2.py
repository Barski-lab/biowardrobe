...
FP_W=open('EA_PXK_risk_wt.fa','r');
x=FP_W.readline().split(' ');
WILD_T=FP_W.readline();#Wild type sequence
FP_W.close();

FP_M=open('EA_PXK_risk_mut.fa','r');
FP_M.readline();
MUT_T=FP_M.readline();#mutation sequence
FP_M.close();

FP_MUT=open('EA_PXK_risk_list.txt','r')
FP_MUT.readline();
...
#		if COORDS[i]>R_COORDS[j]+READ_LEN+len(WTS[i]):
                if pos2-READ_LEN>0:
			j1=j;
			continue;
#		if COORDS[i] < R_COORDS[j]-len(WTS[i]):
                if pos2+corr<0:
			break;
...
#		if M_COORDS[i]>M_R_COORDS[j]+READ_LEN+len(WTS[i]):
                if pos-READ_LEN>0:
			j2=j;
			continue;
#		if M_COORDS[i] < M_R_COORDS[j]-len(WTS[i]):
                if pos+corr<0:
			break;
